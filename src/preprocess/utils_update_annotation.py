...
def check_missing_step_annotation(examples):
    ...
    for example in examples:
        for step in example["step_annotations"]:
            ...
            # missing step => -1
            if "errors" in step and "Missing Step" in [x["tag"] for x in step["errors"]]:
    ...
    for example in examples:
        ...
        for step in example["step_annotations"]:
            if steps_in_recipe.count(step["description"]) == 1:
                ...
                adjusted_step_id = step["step_id"] - activity_id2min_step_id[activity_id]
            ...
            if (example["recording_id"] == "2_42" and ids.count(5) == 2) or (
                ...
                for step_id, descriptions in count.items():
                    ...
                    if len(descriptions) > 1:
                        ...
                        if [x for x in recipe["steps"].values()].count(description) == 1:
                            pass
                        else:
                            logging.error(
