...
def main(args):
    ...
    id2sample = load_frame(args.dirpath_image, args.max_frames)
    ...
    for idx, example in tqdm(enumerate(examples), total=len(examples)):
        ...
        content += get_image_content(
            model_id=args.model_id,
            image_paths=id2sample[example["example_id"]]["paths"],
        )
        # call api
        ...
        new_example["prediction"] = {
            "prompt": text_prompt,
            "frame_ids": id2sample[example["example_id"]]["ids"],
            "rate_inverse": id2sample[example["example_id"]]["rate_inverse"],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_frames", type=int, help="max frames to feed", default=250)
